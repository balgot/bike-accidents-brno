 ...
 * @param {String} where query path
 * @param {Number} width
 * @param {Number} height
 * @param {Number} margin
 */
const drawDonut = (data, attribute, where, width, height, margin) => {
    ...
    const counts = Object.keys(occurrences).map((key) => [Number(key), occurrences[key]]);
